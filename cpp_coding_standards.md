...
* 类、函数、成员变量等实体，采用Doxygen格式的`/** ... */`，在实体上方。
...
* 函数命名应采用`camelCase`的格式，即第一个单词首字母小写，其余单词首字母大写，其他字母小写，单词间直接相连。
...
* 类的函数顺序？（拿走）
* 函数参数顺序？（是应该顺便放在这里，还是单独拆开来）
* 严禁函数throw声明（这个显然应该和顺序一起拿走）。
...
对于任何需要通过`if`判断的代码，应通过`if`/`else if`将所有可能出现的情况枚举出来，相应处理，并用`else`表达不应该出现的情况。

注意避免使用有“且”、“或”的条件。如果要使用，需要十分仔细地分析其他的`else if`分支有没有不小心漏数了一些情况。

反对：

* 多层无`else`甚至无`else if`的裸`if`块。
