...
* 新写代码时，应养成先写注释表达意图和思路，再写代码的习惯。
* 未完成或测试性的代码，应用`TODO`或者`FIXME`开头的注释加以说明。
...
* 函数体内的一行或一小段代码，采用`//`开头，空一格后写注释的内容。位置应在代码段上方，反对在代码段后端的写法。
* 注释应适度进行局部的对齐。
...
* 先行注释表达意图、思路和待办事项，要比完整正确地用代码实现容易，所以如果写代码时遇到困难陷入细节或者切换上下文去写别处代码，预先写的注释在下次接着写时就能起到很好的提示作用。
* 格式混乱的注释，影响阅读观感和代码品质。
...
* 不应用一个类似`common_include.h`的文件来间接包含项目的普遍依赖项，并在每一个头文件中包含它。这样做会放大依赖，引入未知的依赖问题，且降低编译速度。
...
```
另外一种namespace自身不缩进的写法：

```cpp
namespace outer { namespace inner {  // 良好的风格：namespace自身不缩进，多层合并一行

void foo() {                         // 良好的风格：不因在namespace内而增加缩进深度
  ...
}

} } // namespace outer::inner        // 良好的风格：在尾部的花括号注释说明对应的多层namespace
```
...
* 原则上，一个组织/一个子系统应该有自己的顶级命名空间，下设以模块、功能的次级命名空间，建议1层，一般不超过2层。
* 命名空间的命名规则同文件。
...
* 坚定一致地使用作用域前缀：
  - 成员变量，应采用`m_`前缀（因此使用时不再需要通过`this->`来区分成员变量与参数、局部变量）。注意：该前缀仅适用于非`public`的成员，因此结构体的成员变量不应有`m_`前缀。
  - 全局变量，应采用`g_`前缀（但应规避全局变量）。
  - 静态变量，应该用`s_`前缀。
  - 全局的共享内存变量，可采用`gg_`前缀（存在争议）。
  - 具备上述多个作用域的，按照`sgm`的顺序（符合英语语序）进行结合。
  - 作用域前缀__不__算在`camelCase`的格式之内。
...
* 类型前缀算在`camelCase`的格式之内。

注意：作用域前缀永远在类型前缀之前，如一个表达`foo`这个意思的、类型为函数指针的成员变量，加了作用域前缀和类型前缀后应命名为`m_pfnFoo`。
...
* 对于访问和修改类的函数，应分别使用`get`和`set`前缀（首字母的大小写和其他函数保持一致），其中访问类函数的`get`可以略去，但需要在一个类里乃至一个代码基里对此保持一致。访问类函数应声明为`const`，见FB.1。
...
* 避免使用友元函数和友元类（`friend`关键字），考虑调整类之间的关系，而非加大数据暴露。
* 避免污染全局命名空间：减少`using namespace xxx`的使用。头文件中禁用，定义文件中优先考虑直接在用时加命名空间前缀，其次考虑使用`using xxx::yyy`。
* 局部变量的作用域应尽可能小。
* 成员变量和成员函数，应尽可能减少可访问它的范围。
* 可采用如下两种方案之一，区分成员函数调用和全局函数调用：
  - 成员函数调用前不加`this->`，全局函数前调用加`::`
  - 成员函数调用前加`this->`，全局函数调用直接调用
...
* 捕获异常时应用const引用。
* 不要将整数、字符串等基础类型作为异常抛出，如：`throw (errno)`。
...
* 严禁使用printf、`std::cout`、`std::cerr`等方式打印，即使需要打印到标准输出/标准错误端，也应通过日志库来配置。程序运行起始应先初始化日志，初始化日子库失败可以回退到上述方式打印到标准输出/标准错误端。
* 本地日志应使用高性能（如异步写文件、高并发下线程安全）、稳定（代码质量过硬、经过实战考验趋于成熟、活跃维护中）、易定制（如每行开头的时间等信息、日志按天或按大小滚动）、轻量（代码量少可以通读代码对其充分掌握；依赖库少更独立）的日志库打印。
* 远程日志应使用带有本地缓冲且异步化的方式发送到远程日志服务器。

日志打印的内容、数量应合理化：

* 以下信息应体现为可被在代码运行时的黑盒外感知的可观测项，如果没有间接通过响应报文、持久化存储的数据、监控告警、抛异常等方式体现出来，就应通过日志体现出来：
  - 调用底层库API的返回值、报错信息
  - 所进入的关键的特殊分支和异常分支
  - 程序的关键参数和进入的关键状态
  - 其他如果日志中没有，定位问题的时候会非常抓瞎以至于需要单独上打印日志的版本的信息
* 日志打印应进行合理分级，并控制量级，至少使用如下几个级别：
  - `ERROR`：程序出现严重问题 —— 这是追求性能情况下，生产系统设置的日志级别，数量要少之又少
  - `INFO`：程序正常运行的关键参数、状态等 —— 这是一般情况下生产系统设置的日志级别，要控制数量不能泛滥
  - `DEBUG`：程序在开发阶段或者问题定位阶段打印的日志 —— 这些并不是开发临时打印，而要为日后定位问题服务，内容上也不能随意，比如打印“test1”、“test2”等来找程序跑到哪悲剧的
* 日志应在一定程度上结构化：
  - 每行开头应有固定的时间、文件、行号等内容，由日志库提供
  - 随后应有结构化信息，如调用类型、时耗、错误码等，由应用框架提供
  - 最后才是仅供人类阅读的文本内容
  - 每行日志应提供独有的关键字，方便搜索过滤
  - 仅供人类阅读的文本内容允许换行，但如果有日志结构化解析的程序，应考虑该情况
  - 允许一个文件中有多于一种结构化方式，但要能让日志结构化解析的程序识别的标志
* 不应打印如下内容的日志：
  - 日志内容固定而没有变化的上下文信息，如“进入XXX函数/XXX步骤”、“出错了！”
  - 日志描述与实际运行状态不一样，或不易看懂（如语法错乱、用词不当的英文），从而形成误导
  - 和即将抛出的异常信息相近的冗余日志（应交给异常捕捉者统一打印）
  - 抖机灵、调侃、发泄等无意义内容
...
* 成员函数/变量，应按`public`、`protected`、`private`的顺序声明。
* 成员变量的命名规范见 N.5 。
* 成员函数的命名规范见 N.6 ，参数规范见 FB.1 ，尤其注意不修改成员变量的函数，应标注为`const`。
* 对于逻辑上不能在初始化后更改的成员变量，不应提供可修改它的成员函数（即“Setter函数”）。如果对一个成员变量提供了Setter函数，Setter函数应确保更新该成员变量变化之后，关联的其他成员变量得到更新，关联的动作得到执行，且对象的行为不会出现前后不一致等偏差。
* 对于非`public`的成员函数，可以考虑一律放在`protected`区，以供子类调用。
* 对于非`public`的成员变量，可以考虑一律放在`private`区，以防子类误修改或者过度依赖，如果希望为子类提供访问/修改的便利，可以考虑提供访问/修改的成员函数（`public`或`protected`）。
...
> 注：团队可在此添加自己定制的规则，这里给出两个团队的定制规则作为样例：

团队A:

* F.4 一致地使用4个空格
* F.5 一致地使用“break”风格进行花括号换行
* N.6 函数命名应采用`CamelCase`的格式，即每个单词的首字母都__大写__，其他字母小写，单词间直接相连
* B.4 待定
...
* F.4 一致地使用4个空格
* F.5 一致地使用“break”风格进行花括号换行
* N.6 函数命名应采用`camelCase`的格式，即第一个单词首字母__小写__，其余单词首字母大写，其他字母小写，单词间直接相连
* B.4 待定
