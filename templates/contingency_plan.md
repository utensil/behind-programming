...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 此时间前必须完成 | 变更的具体内容 | 如：发版本、改配置等 | 希望达成的正面目的 | 不变更可能带来的负面损失与影响 | 负责变更的人员 | 变更前后需要知会的人员 | 核验的实际完成时间与结果：符合预期、不符合预期、调整方案等|

### 核验确保

| 时间 | 事项 | 方式 | 目的 |  影响  | 例外  |  负责人 | 知会人 | 结果记录 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 此时间前必须完成 | 核验的具体内容 | 如：核对脚本、观察监控等 | 希望达成的正面目的 | 不核验可能带来的负面损失与影响 |  该应确保项的例外，及例外的合理性与影响分析 |  负责核验的人员 | 核验前后需要知会的人员 | 核验的实际完成时间与结果：符合预期、不符合预期、调整方案等|
...
| 模块 | 触发条件 | 影响分析 |  处理方式/时效 | 处理步骤 | 最晚执行时间 | 负责人 | 知会人 | 结果记录 |
| --- | --- | --- | --- | --- | --- | --- | --- |--- |
| 或子业务、子系统 | 除默认触发条件外的触发条件，或默认触发条件对应的瓶颈项 | 对业务/活动的影响评估  | 自动（秒级）/手动(分钟级) | 如发管理指令、修改配置、核对脚本、观察监控等的具体操作指引 | 达到该最晚时间，如无问题，该预案措施均执行 |  负责预案的人员 | 预案操作前后需要知会的人员 | 预案的结果记录 |


风险预案
--------------------

| 模块 | 风险场景 | 影响分析 | 处理方式/时效 | 处理步骤 | 负责人 | 知会人 | 结果记录 |
| ---| --- | --- | --- | --- | --- | --- | --- |
| 或子业务、子系统| 如单机故障、单IDC故障、成功率下降、耗时上升等| 对业务/活动的影响评估 | 自动（秒级）/手动(分钟级) | 如发管理指令、修改配置、核对脚本、观察监控等的具体操作指引 | 负责预案的人员 | 预案操作前后需要知会的人员 | 预案的结果记录 |

归因预案
----------------------

> 为了当某个模糊现象出现时，尽快根据一些监控告警等细致现象，将现象进行归因，从而对应到某个瓶颈或风险

| 现象 | 归因依据 | 判断标准 | 对应预案 |
| --- | --- | --- | --- |
| 某一类模糊的现象 | 一些辅助判断的监控指标、告警 | 每种细致现象的判断条件、标准 | 对应的风险/瓶颈预案 |
...
> 活动虽已结束，在去开展其他项目之前，仍需进行的复原变更，如：恢复活动期间一些超频调整
...
> 活动虽已结束，在去开展其他项目之前，仍需观察核验的后置条件，如：跑一些数据对账、出一些活动观察报告等
